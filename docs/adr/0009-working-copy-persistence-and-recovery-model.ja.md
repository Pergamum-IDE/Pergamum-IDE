...
10. **実装状況（as of Phase 7-2 / 2026-09-01）。** 上記 1〜9 は 2026-08-27 時点の調査であり、その後 Phase 7-2 までに変わった点・変わらない点を明確化する。
    - **実装済み（現行の Recovery restore 経路）**: dirty payload の capture（base fingerprint `baseSize` / `baseSha256` を記録）、`origin_instance_run_id` による previous-run 判定と起動時の候補一覧／auto-show／reminder、`recoveryId` をキーにした**フラットな候補リスト**、restore（候補本文を**新規の `.recovered[-N].md` sidecar** に atomic write。**元ファイルは上書きしない**。既存の `.recovered` も上書きしない）、**2 フェーズ finalize**（restore モジュールは書くだけで Recovery 行を削除しない → renderer が書かれたファイルを開く → その後 `recovery:finalizeRestoredCandidates` が行を削除する。**行削除は restore finalize の後**）。実装コード上のラベルは "Phase 6-4-4"。
    - **未実装（本 ADR は契約のみ。現行動作ではない）**: reconciliation classification engine（S-42〜S-46）、S-45 の判定手順、reconciliation のタイミング（S-51 / S-52）、save-attempt lifecycle と retirement（S-27、`saveAttemptFingerprint` / `saveAttemptToken`）、per-snapshot exclusive claim（S-53〜S-56）、external-change detection（S-19 の advisory hint 消費側。項目 4 参照）、原本上書きによる reconciliation。記録済みの base fingerprint を消費して分類する処理は存在しない。
    - **Status: Accepted の意味**: 本 ADR は「working copy 単位の Recovery **契約**」として採択済みという意味であり、S-* の全手順が現時点で実装済みという意味ではない。上記の実装済み範囲のみが shipped である。
    - **ADR-0010（起動時ファイルオープンのルーティング）との関係**: ADR-0010 は本 Recovery 契約を**変更しない**。起動時 Markdown ルーティングは Recovery snapshot の分類・claim・順序に影響しない。起動時ルーティングの rejection dialog / read-only confirmation は、modal 衝突を避けるため cold-start restore の他 Error dialog と同じ deferred-dialog idle boundary から提示される（Recovery candidate dialog との**絶対順序は ADR-0010 では規範化しない**。ADR-0010 §Implementation notes）。multi-instance 環境での Recovery owner に関する既知の限界は ADR-0010 KNOWN-3 に記載。
